...
## The loop
1. **Arm the ear** as a persistent monitor (never a hand-rolled polling loop): Windows `pwsh -File <bundle>\livemind-lite\lm-ear.ps1`, Unix `<bundle>/livemind-lite/lm-ear`. In Claude Code: `Monitor({ command: "<that command>", persistent: true, description: "mini-mouth ear" })`. It prints `selftest:` replays of the last real lines and `EAR ARMED …` before the first live line — that is your proof it works (`--selftest` alone: exit 0 = the filter fires on real lines).
