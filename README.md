...
- **Windows**: [v0.1.2 release](https://github.com/fire17/mini-mouth-handoff/releases/tag/v0.1.2), with the corrected r4 bundle and [setup instructions](SETUP-WINDOWS.md). Launchers accept the driver's existing Codex login path, and setup reuses Python 3.11 or newer.
...
Windows package installation and imports have been tested on a real Windows 11 machine with Python 3.11.9. The owner's Codex login connected to realtime, and the player consumed a complete generated speech buffer. Audible output and a microphone conversation remain to be verified with the owner. See the setup guide for platform diagnostics and the Caps Lock limitation.
