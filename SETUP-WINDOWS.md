...
The installer uses winget for Python 3.12, FFmpeg/ffplay, and Memurai Developer when missing. Windows may show an installer approval dialog. It creates `mini-mouth\.venv`, installs both bundled Python packages there, verifies imports, and opens the mouth. System packages retain their own licenses; Memurai Developer is intended for development. If you already have Redis listening on `127.0.0.1:6379`, choose `-RedisMode Existing` instead. With Docker Desktop already running, choose `-RedisMode Docker`; the container binds to loopback only.

Enter your own OpenAI API key when prompted. The prompt is hidden; the launcher keeps the key only in its process environment, without putting it in the command line, files, or persistent user environment. An existing `OPENAI_API_KEY` environment variable is also accepted.
